...
__global__ void kernUpdateVelNeighborSearchScattered(
    ...
    // find the bounding box of A GRID
...
void Boids::stepSimulationNaive(float dt) {
    ...
    dim3 blocksPerGrid((numObjects + blockSize - 1) / blockSize); // 分块，每块的大小是[n / blockSize]，向上取整，所以(numObjects + blockSize - 1) /  blockSize
...
void Boids::stepSimulationScatteredGrid(float dt) {
    ...
    kernIdentifyCellStartEnd<<<blocksPerGridCells, blockSize>>>(numObjects, dev_particleGridIndices, dev_gridCellStartIndices, dev_gridCellEndIndices);

    kernUpdateVelNeighborSearchScattered<<<blocksPerGridBoids, blockSize>>>(numObjects, gridSideCount, gridMinimum, gridInverseCellWidth, gridCellWidth,
...
void Boids::stepSimulationCoherentGrid(float dt) {
    ...
    kernSortPosAndVel<<<blocksPerGridBoids, blockSize>>>(numObjects, dev_particleArrayIndices, dev_pos, dev_sortedPos, dev_vel1, dev_sortedVel);

    kernUpdateVelNeighborSearchCoherent<<<blocksPerGridBoids, blockSize>>>(numObjects, gridSideCount, gridMinimum, gridInverseCellWidth, gridCellWidth,
