...
__global__ void kernUpdateVelNeighborSearchScattered(
	...
	for (auto cell : adjacentCells)
	{
		...
		for (int bp = startIndex; bp <= endIndex; bp++)
		{
			...
			if (b != index && glm::distance(pos[b], thisPos) < rule1Distance) {
				perceived_center += pos[b];
				neighborsRule1++;
			}
			// Rule 2: boids try to stay a distance d away from each other

			if (b != index && glm::distance(pos[b], thisPos) < rule2Distance) {
				c -= (pos[b] - thisPos);
			}

			// Rule 3: boids try to match the speed of surrounding boids
			if (b != index && glm::distance(pos[b], thisPos) < rule3Distance) {
	...
	if (neighborsRule1 > 0)
	{
		...
		v1 = (perceived_center - thisPos) * rule1Scale;
	}
...
void Boids::stepSimulationScatteredGrid(float dt) {
	...
	kernUpdateVelNeighborSearchScattered << < fullBlocksPerGrid, blockSize >> > (numObjects, gridResolution, gridMinimum, gridInverseCellWidth, gridCellWidth, 
